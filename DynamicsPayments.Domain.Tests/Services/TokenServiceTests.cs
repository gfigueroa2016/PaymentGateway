...
using DynamicsPayments.Repositories;
...
{
    public class TokenServiceTests : IClassFixture<PaymentGatewayContextFactory>
    {
        private readonly IDynamicsPaymentsTokenService _dynamicsPaymentsTokenService;
        public TokenServiceTests(IDynamicsPaymentsTokenService dynamicsPaymentsTokenService)
        {
            _dynamicsPaymentsTokenService = dynamicsPaymentsTokenService;
        }
        [Fact]
        public async Task GetPaymentTransactionsHistory_Should_Return_Right_Data()
        {
            var result = await _dynamicsPaymentsTokenService.IENumerable<GetCustomerTokensAsync>(GetCustomerTokensRequest getCustomerTokenRequest);
